...
public class AdminService {
    ...
    //添加服务
    public boolean addFruitItem(String number,String fruitname,String price,String unit) {
        //调用Dao层获取所有数据方法获取所有数据
        ...
        //如果没有重复编号，将数据 封装为FruitItem对象
        FruitItem thisFruitItem = new FruitItem(number,fruitname,Double.parseDouble(price),unit);
        //调用Dao层的添加数据
    ...
    //修改服务
    public boolean updataFruitItem(String number,String fruitname,String price,String unit) {
        //调用Dao层获取所有数据方法获取所有数据
        ArrayList<FruitItem> data = queryFruitItem();
        //使用输入的编号与所有数据对比
        for (int i = 0; i < data.size(); i++) {
